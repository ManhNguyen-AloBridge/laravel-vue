...
function renderHeaderBreadsScrum(step) {
	...
	const routePrev = $('.tabs-block').attr('data-prev-route');
	const isFromSettingCompanyScreen = routePrev === 'setting.company';
	const urlPrev = $('.tabs-block').attr('prev-url');
	let currentUrl = window.location.href;
	let header = [
		{
			title: isFromSettingCompanyScreen
				? i18next.t('layout.header.setting_company.info')
				: i18next.t('layout.header.profile.list_staff'),
			route: urlPrev,
		},
		{
	...
	if (step == 3) {
		...
		header.push({
			title: i18next.t(
				'layout.header.profile.confirm_update_personal_info'
			),
		});
...
function changeInnerTitleContentPage(step) {
	...
	if (step == 2) {
		...
		<h1 class="m-0 text-2xl">${i18next.t(
			'layout.header.profile.update_personal_info'
	...
	if (step == 3) {
		...
		$('.main-content-heading-main__note').append(
			`<p>${i18next.t('pages.staff.note_title')}</p>`
		);
	...
	innerTitle.append(`
	<h1 class="m-0 text-2xl">${i18next.t(
		'layout.header.profile.detail_personal_info'
	)}</h1>
		`);
}
